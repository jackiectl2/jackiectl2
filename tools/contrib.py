#!/usr/bin/env python3
"""Render the contribution graph into assets/contributions-{dark,light}.svg.

Why this exists instead of a card service: the hosts we tried keep dying
(github-readme-stats, github-profile-trophy), and a card that fails serves its error page
with a day-long cache header, so a bad minute upstream becomes a bad day on the profile.
This pulls the numbers from GitHub's GraphQL contributionsCollection and commits the SVGs.

cards.active_graph picks the shape — `local_daily` draws an area chart over the last `days`,
`local_monthly` draws bars over the last `months`. Either way the window rolls: it ends
today. Regenerated by .github/workflows/stats-cards.yml.
...
def main():
    ...
    if kept_existing:
        print("kept %d existing graph(s); candidate %s graph (%s) has %d contributions"
              % (kept_existing, mode, label, total))
    else:
