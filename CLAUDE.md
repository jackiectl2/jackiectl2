...
**刷 28 天, 空 1 天, 无限循环.** 由 `Decide whether today is a rest day` 这一步实现,
后面 6 步全部挂 `if: steps.gate.outputs.skip != 'true'`.

- 🔴 **cron 表达不了 29 天周期** —— day-of-month 绑日历, 而月份是 28~31 天, 任何固定日期集合都会漂.
  所以周期位置是**从固定锚点 `2026-07-23` 算天数差再取模**: 无状态文件, 无需重置, 永远不漂.
- 29 不是 7 的倍数 → 休息日**在星期之间漂移**, 不会永远卡在同一个星期几. 已验证前 8 个休息日是
  Thu/Fri/Sat/Sun/Mon/Tue/Wed/Thu.
- **只有 `schedule` 会休息.** `push` 和 `workflow_dispatch` 是人主动要刷新, 休息日照样跑.
- 已按脚本原文模拟验证: 第 1/14/28 天 `skip=false`, 第 29 天 `skip=true`,
  第 29 天遇 push/dispatch 仍 `skip=false`, 锚点之前的日期也对 (Python 取模非负).
...
`tools/*.py` 用的是环境里的 `GITHUB_TOKEN`. 本机 `gh auth token` 是 **fine-grained** 的,
按官方文档只能看到公开数据 —— 本地生成的卡会把 Total PRs 从 19 打回 2, Total Commits 少几百.
**卡片只由 Action 生成** (那里才有 classic 的 `STATS_PAT`). 本地只用 `--check` 看新旧, 别写盘提交.
